...
public class VivoSignUtils {
    ...
    public static void main(String[] args) {
        ...
        params.put(APP_ID_PARAM, "123");
        params.put(CP_ORDER_NUMBER, "123456878");
        params.put(EXT_INFO, "扩展参数");
        params.put(NOTIFY_URL, VivoConfig.payBackUtl);
        params.put(ORDER_AMOUNT, "1000");
        params.put(PRODUCT_DESC, "vivo Xplay");
        params.put(PRODUCT_NAME, "手机");

        // 角色信息
        params.put(BALANCE, "用户余额");
        params.put(VIP, "用户vip等级");
        params.put(LEVEL, "用户角色等级");
        params.put(PARTY, "工会");
        params.put(ROLE_ID, "角色ID");
        params.put(ROLE_NAME, "角色名");
        params.put(SERVER_NAME, "区服信息");

        String key = "123456789123456";

        String sign = getVivoSign(params, key);
        System.out.println("vivo签名值：" + sign);

        boolean verify = verifySignature(params, key);
