#ifndef _WORLD_CURVED_FUNC_HLSH_
#define _WORLD_CURVED_FUNC_HLSH_
...
inline void V_CW_TransformPointAndNormal(inout float4 vertex, inout float3 normal, float3 worldPos, float3 worldTangent, float3 worldBinormal,float _CurveFactor = 1)
{
	float3 v0 = worldPos;
	v0 -= PIVOT;
	...
	xyOff *= xyOff;
	float3 transformedVertex = float3(_V_CW_BendAxis.x * xyOff.x, -_V_CW_BendAxis.y * xyOff.y, 0.0f) * 0.001*_CurveFactor*_V_CW_Curve;
	v0 += transformedVertex;


	xyOff = max(_zero2, v1.zz - _V_CW_BendOffset.xy);
	xyOff *= xyOff;
	v1 += float3(_V_CW_BendAxis.x * xyOff.x, -_V_CW_BendAxis.y * xyOff.y, 0.0f) * 0.001*_CurveFactor*_V_CW_Curve;


	xyOff = max(_zero2, v2.zz - _V_CW_BendOffset.xy);
	xyOff *= xyOff;
	v2 += float3(_V_CW_BendAxis.x * xyOff.x, -_V_CW_BendAxis.y * xyOff.y, 0.0f) * 0.001*_CurveFactor*_V_CW_Curve;


	vertex.xyz += mul((float3x3)UNITY_MATRIX_I_M, transformedVertex);
	normal = normalize(mul((float3x3)UNITY_MATRIX_I_M, normalize(cross(v2 - v0, v1 - v0))));
	/*
	float3 v0 = worldPos;
	float3 v1 = v0 + worldTangent;
	float3 v2 = v0 + worldBinormal;
	...
	float2 angle = _V_CW_Angle * SIGN2(radius)*_CurveFactor*_V_CW_Curve;
...
inline void V_CW_TransformPointAndNormal(inout float4 vertex, inout float3 normal, float4 tangent,float _CurveFactor =1)
{
	...
	V_CW_TransformPointAndNormal(vertex, normal, worldPos, worldTangent, worldBinormal, _CurveFactor);
}

inline void V_CW_TransformPoint(inout float4 vertex,float _CurveFactor = 1)
{
	...
	xyOff *= xyOff;
	worldPos = float4(_V_CW_BendAxis.x * xyOff.x, -_V_CW_BendAxis.y * xyOff.y, 0.0f, 0.0f) * 0.001*_CurveFactor*_V_CW_Curve;

	vertex += mul(UNITY_MATRIX_I_M, worldPos);
...
#define CURVED_WORLD_TRANSFORM_POINT_AND_NORMAL(vertex,normal,tangent, factor) V_CW_TransformPointAndNormal(vertex, normal, tangent,factor);
#define CURVED_WORLD_TRANSFORM_POINT(vertex,factor)                           V_CW_TransformPoint(vertex,factor);
