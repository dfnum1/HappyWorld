...
FDirectLighting MaskMergeLit(Surface surface, real3 N, real3 V, real NoL, Light l, real mask)
{
    BxDFContext context;
    InitBxDFContext(context, N, V, l.direction);

    real3 falloffColor = CalMergeFalloffColor(context, NoL, l, surface, mask >= 1.0);
    ...
    Lighting.Diffuse = falloffColor * CalDiffuse(surface, context, NoL);// * M_PI;

    real3 specTerm = EnvBRDFApprox(surface.specColor, surface.roughness, context.NoV);

    Lighting.Specular = falloffColor * MergeSchlickBackMannSpec(surface, surface.roughness, specTerm, context, NoL, l, mask <= 0.0) * surface.specOcclusion;
	
    //real3 hairSpec = CalHairShadingSpec(falloffColor, context, surface, 0.5, real2(0.01, 0.05), 0.16, 1.0, normalize(l.direction + N));
    //if (mask <= 0.0)
	//Lighting.Specular = hairSpec;
...
FDirectLighting MixLit(Surface surface, real3 N, real3 V, real NoL, Light l)
{
	...
	real3 falloffColor = CalFalloffColor(context, NoL, l, surface);
