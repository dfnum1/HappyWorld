...
real		environment_exposure;

/* // substance painter SH Matrix work. remove.
//: param auto environment_irrad_mat_red
uniform real4x4	irrad_mat_red;
//: param auto environment_irrad_mat_green
uniform real4x4	irrad_mat_green;
//: param auto environment_irrad_mat_blue
uniform real4x4	irrad_mat_blue;

real3 envIrradianceInMatrix(real3 dir)
{
	real4 shDir = real4(dir.xzy, 1.0);

	real3 final = real3(
		dot(shDir.xyz, mul(shDir, irrad_mat_red)),
		dot(shDir.xyz, mul(shDir, irrad_mat_green)),
		dot(shDir.xyz, mul(shDir, irrad_mat_blue)));

	return max(real3(0.0, 0.0, 0.0), final) * environment_exposure;
}
*/
...
real3 envSampleLODTest(real3 reflectDir, real lod)
{
	...
	return GammaToLinearSpace(col) * environment_exposure;
#else
	return col.rgb * environment_exposure;
#endif
}
...
#if defined(UNITY_COLORSPACE_GAMMA) && !defined(USE_PRE_GAMMA2LINEAR)
	return GammaToLinearSpace( SAMPLE_TEXTURECUBE_LOD_DEF(_EnvCubeMap, shDir.xyz, lod).rgb ) * environment_exposure;
#else
	return SAMPLE_TEXTURECUBE_LOD_DEF(_EnvCubeMap, shDir.xyz, lod).rgb * environment_exposure;
#endif	
}
#endif
...
//	skyData = skyData.rbg; // in unity urp define. this is real unbeliveable
	...
	return skyData.xyz * environment_exposure;
...
real3 envIrradianceInSH9(real3 dir)
{
	...
	return sh9 * environment_exposure;
}
#endif
...
real3 BuildSH9Vertex(real3 normalWorld)
{
	...
	shResult.rgb += SHEvalLinearL2(normalWorld, SHBr, SHBg, SHBb, SHC);
#else
...
#if  defined(USE_ENV_CUBE2D) || defined(USE_ENV_CUBE)
	shResult.rgb = SHEvalLinearL0L1(normalWorld, SHAr, SHAg, SHAb) + vertexSH;
#else
...
{
#if defined(USE_ENV_CUBE2D)
	return 0;// envSampleLODInMatcap(dir, lod);
#elif defined(USE_ENV_CUBE)    
...
real3 GetImageBasedReflectionLighting(real roughness, real3 reflectDir, real4 diffuseGI)
{
	...
	real CubemapMaxMip = maxLod;//UsingSkyReflection ? MobileReflectionParams.w : ResolvedView.ReflectionCubemapMaxMip;
