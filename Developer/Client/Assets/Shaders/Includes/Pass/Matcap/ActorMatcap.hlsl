...
VertexToFragment MatcapVS(FragmentToVertex v)
{
	...
	float rotation = _MatCapRotation * 3.14159265359 / 180.0;
	float rotCos = cos(rotation);
	float rotSin = sin(rotation);
	float2 rotatedNormalXY = float2(
        rotCos * output.diffuseUVAndMatCapCoords.z - rotSin * output.diffuseUVAndMatCapCoords.w,
        rotSin * output.diffuseUVAndMatCapCoords.z + rotCos * output.diffuseUVAndMatCapCoords.w);
	
	//归一化的法线值区间[-1,1]转换到适用于纹理的区间[0,1]
	output.diffuseUVAndMatCapCoords.zw = rotatedNormalXY.xy * 0.5 + 0.5;
...
float4 MatcapFS(VertexToFragment input) : COLOR
{
	...
	float4 finalColor = float4(mainColor * _MainColor.rgb * _ProjectorColor.rgb, _MainColor.a);
        ...
        finalColor = lerp(finalColor, _ShadowColor, (1.0 - shadowAttenutation) * argvLight);
    #endif
