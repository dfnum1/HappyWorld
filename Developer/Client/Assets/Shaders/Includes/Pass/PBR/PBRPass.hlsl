...
real4 ActorFrag(Varyings i) : SV_TARGET
{
	...
	BuildSurfaceAnisotropy(s, 1.0, MetalRoughMask.z <= 0 ? 1.0 : 0.0);

	real4 envCol = BuildAmbient(i.normalWorld);
	...
	//BuildSurfaceColor(s, baseColor, 1.0, envCol, 0, 1, lerp(1.0, normalRGAO.z, _AOStrength), 0.01);
...
//#endif
	...
	//Light sphereLight = SphereLightInit(s, _SphereLight0Pos.xyz, _SphereLight0Radius, _SphereLight0Color);
	...
	//real NoL_1 = max(0, dot(s.normalWorld, sphereLight.direction));

	//FDirectLighting lighting_1 = MaskMergeLit(s, s.normalWorld, s.viewDir, NoL_1, sphereLight, MetalRoughMask.z);
