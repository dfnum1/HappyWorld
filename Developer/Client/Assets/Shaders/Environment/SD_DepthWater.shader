Shader "SD/Environment/SD_DepthWater"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            uniform sampler2D _LastColorTexture;
            uniform sampler2D _LastDepthTexture;
            ...
            v2f vert (appdata v)
            {
                ...
                // o.grabPosition = ComputeGrabScreenPos(mul(UNITY_MATRIX_VP, float4(mul(UNITY_MATRIX_M, v.vertex).xyz, 1)));
            ...
            float4 frag (v2f i) : SV_Target
            {
				...
				float grabSign = _ProjectionParams.x;
#endif
				screenCoord.y *= _ProjectionParams.x;
                ...
                float3 fragColor = tex2D(_LastColorTexture, screenCoord.xy).rgb;
                float fragDepth = tex2D(_LastDepthTexture, screenCoord.xy).x;//_LastDepthTexture,_CameraDepthTexture
                ...
                float opticalDepth = abs(LinearEyeDepth(fragDepth,_ZBufferParams) - LinearEyeDepth(i.vertex.z,_ZBufferParams));
         ...
         //           shadowMask = GetLightVisibility(_MainDirectionalShadowMap, i.worldPosition, _MaxShadowDistance);
          //      #endif
				...
				Util_AmbientColor(baseColor, _AmbientStrength);
                ...
                float3 reflectedColor = float3(0,0,0);//UNITY_SAMPLE_TEXCUBE(unity_SpecCube0, reflect(viewDirWS, normalWS));
               ...
               // sssColor *= saturate(dot(viewDirWS, _WorldSpaceLightPos0.xyz));
                ...
                float sunSpecularMask = 0;// saturate(dot(viewR, _WorldSpaceLightPos0));
                ...
                sparkleMask = 0;//ceil(saturate(pow(sparkleMask, _SparkleExponent))) * shadowMask * distanceMask;
