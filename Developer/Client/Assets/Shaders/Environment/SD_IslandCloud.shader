Shader "SD/Environment/SD_IslandCloud" 
{
	Properties 
	{
	...
	SubShader 
	{
		...
		Pass 
		{
			...
			fixed _FlowSpeed;
			fixed _WriggleSpeed;
			fixed _WriggleMagnitude;
			fixed _WriggleVertexDivergence;
			CBUFFER_END
			...
			fixed procedualTex(fixed2 texcoord)
			{
				...
				fixed perlinNoise = (perlinNoiseF(texcoord * 4.0 + _Time.x * 10.0) + 1) * 0.5;
				//fixed perlinNoise = (ClassicPerlinNoise(texcoord * 4.0 + _Time.y) + 1) * 0.5;
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				return fixed4(_CloudColor, alpha);
			}
			ENDHLSL 			
		}
	}
