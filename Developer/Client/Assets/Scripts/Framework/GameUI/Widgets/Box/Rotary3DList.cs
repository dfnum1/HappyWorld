...
/// 仿3D轮转图组件
/// </summary>
[AddComponentMenu("UI/Rotary3DList", 50)]
...
public class Rotary3DList : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    /// 移动到下一个下标位置
    /// </summary>
    public void LeftMoveToIndex(bool isScroll = true)
    {
        // if (index < 0
        //     || index >= m_ItemCount)
        // {
        ...
        // int indexOffset = CenterIndex - index;
        float offset = rotaryType == RotaryType.Horizontal
            ? (spacing +m_Prefab.GetComponent<RectTransform>().rect.width)
            : (spacing +m_Prefab.GetComponent<RectTransform>().rect.height);
    ...
    /// 移动到上一个下标位置
    /// </summary>
    public void RightMoveToIndex(bool isScroll = true)
    {
        // if (index < 0
        //     || index >= m_ItemCount)
        // {
