...
namespace TopGame.UI
{
    ...
    public class EventTriggerListener : EventTrigger
    {
        ...
        public static EventTriggerListener Get(UIBehaviour go)
        {
            if (go == null) return null;
            EventTriggerListener listener = go.GetComponent<EventTriggerListener>();
            if (listener == null) listener = go.gameObject.AddComponent<EventTriggerListener>();
            return listener;
        }
        ...
        bool OnATEvent(GameObject pTrans, Base.EUIEventType evtType, UIRuntimeParamArgvs param)
        {
            if(evtType == atEventParam.type)
            {
                if(!string.IsNullOrEmpty(atEventParam.eventName))
                    AgentTreeManager.getInstance().ExecuteEvent((ushort)evtType, atEventParam.eventName, param);
            }
    ...
    public class EventTriggerListenerEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUILayout.Label("AT Event 全局回调:");
            evt.atEventParam.type = (Base.EUIEventType)Framework.ED.HandleUtilityWrapper.PopEnum("回调类型", evt.atEventParam.type);
            evt.atEventParam.eventName = EditorGUILayout.TextField("回调名", evt.atEventParam.eventName);
