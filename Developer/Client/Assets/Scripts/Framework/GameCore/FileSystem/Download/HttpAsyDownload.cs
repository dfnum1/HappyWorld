...
namespace TopGame.Core
{
    public class HttpAsyDownload
    {
        ...
        /// 开始下载
        /// </summary>
        public void Start(string root, string remotelocalFileName, string localFileName, Action<HttpAsyDownload, long> nofity = null, Action<HttpAsyDownload> error = null)
        {
            lock (m_Lock)
            {
                this.Abort();
                ...
                remoteName = remotelocalFileName;
                isDone = false;
        ...
        private void OnDownload()
        {
            try
            {
                ...
                {
                    //尝试下载资源
                    m_Request = (HttpWebRequest)WebRequest.Create(url + remoteName);
                    //Debug.LogError("下载地址: " + url + remoteName);
            ...
            {
                Debug.LogWarning("HttpAsyDownload -\"" + remoteName + "\"download failed!" + "\nMsg" + e.Message);
                UnregisterTimeOut();
        ...
        void OnPartialDownload()
        {
            try
            {
                lock(m_Lock)
                {
                    m_Request = WebRequest.Create(url + remoteName) as HttpWebRequest;
                    m_Request.Timeout = TIMEOUT_TIME;
                    m_Request.KeepAlive = false;
