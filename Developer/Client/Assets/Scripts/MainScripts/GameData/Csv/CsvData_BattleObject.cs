...
namespace TopGame.Data
{
    ...
    {
        Dictionary<long, BattleObjectData> m_vSceneThemes = null;
        Dictionary<long, List<BattleObjectData>> m_vSubTypeDatas = null;
#if UNITY_EDITOR
        Dictionary<int, List<BattleObjectData>> m_vEditorSubTypeDatas = new Dictionary<int, List<BattleObjectData>>();
        ...
        protected override void OnAddData(IUserData baseData)
        {
            ...
            if (m_vSceneThemes == null) m_vSceneThemes = new Dictionary<long, BattleObjectData>();
            for(int i =0; i< objData.sceneThemes.Length; ++i)
            {
                key.intVal0 = (int)objData.sceneThemes[i];
...
#if UNITY_EDITOR
            string descFile = Application.dataPath + "/../EditorData/Runnings/battleObjectSubTypes.txt";
...
#if UNITY_EDITOR
        //------------------------------------------------------
        public List<BattleObjectData> GetEditorSubTypeDatas(byte subType)
        {
            List<BattleObjectData> vObs;
...
#endif
        //------------------------------------------------------
        public List<BattleObjectData> GetSubTypeDatas(byte subType, uint sceneTheme)
        {
            Variable2 key = new Variable2();
        ...
        //------------------------------------------------------
        public BattleObjectData GetData(uint sceneTheme, uint id)
        {
            BattleObjectData objData;
