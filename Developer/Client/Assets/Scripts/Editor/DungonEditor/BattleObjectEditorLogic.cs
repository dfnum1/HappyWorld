...
{
    public class BattleObjectEditorLogic
    {
        public class ObjectItem : TreeAssetView.ItemData
        {
            public CsvData_BattleObject.BattleObjectData pData;
        ...
        //-----------------------------------------------------
        void OnChangeSelect(CsvData_BattleObject.BattleObjectData item)
        {
            if (item == m_pCurItem)
                return;
            ClearTarget();
        ...
        void RefreshList()
        {
            m_pCurItem = null;
            Clear();
            CsvData_BattleObject projectile = DungonEditor.GetDataer().BattleObject;
   ...
   //         m_bDraging = true;
        ...
        public void OnDrawInspecPanel(Vector2 size)
        {
            ...
            if(m_pCurItem!=null)
            {
                ...
                EActorCollisionType type = (EActorCollisionType)EditorGUILayout.EnumPopup("boxType", m_pCurItem.boxType);
                if (GUILayout.Button("复制数据"))
                {
                    EditorKits.CopyToClipboard(((int)type).ToString());
                }
                GUILayout.EndHorizontal();
                if (bCanSet)
                    m_pCurItem.boxType = type;
                if (m_pCurItem.boxType == EActorCollisionType.BOX)
                {
                    GUILayout.BeginHorizontal();
                    Vector3 min = EditorGUILayout.Vector3Field("碰撞盒-min", m_pCurItem.aabb_min);
                    if (GUILayout.Button("复制数据"))
                    {
                        EditorKits.CopyToClipboard(string.Format("{0:F2},{1:F2},{2:F2}", m_pCurItem.aabb_min.x, m_pCurItem.aabb_min.y, m_pCurItem.aabb_min.z));
                    }
                    GUILayout.EndHorizontal();
                    GUILayout.BeginHorizontal();
                    Vector3 max = EditorGUILayout.Vector3Field("碰撞盒-max", m_pCurItem.aabb_max);
                    if (GUILayout.Button("复制数据"))
                    {
                        EditorKits.CopyToClipboard(string.Format("{0:F2},{1:F2},{2:F2}", m_pCurItem.aabb_max.x, m_pCurItem.aabb_max.y, m_pCurItem.aabb_max.z));
                    }
                    GUILayout.EndHorizontal();
                    if (bCanSet)
                    {
                        m_pCurItem.aabb_min = Vector3.Min(min, max);
                        m_pCurItem.aabb_max = Vector3.Max(min, max);
                    }
                }
                else
                {
                    float param = EditorGUILayout.FloatField("球半径", m_pCurItem.aabb_min.x);
                    if (bCanSet)
                        m_pCurItem.aabb_min.x = param;
                }
