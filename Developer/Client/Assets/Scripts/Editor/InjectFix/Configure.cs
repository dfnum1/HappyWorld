...
namespace IFix
{
    ...
    public class IFixAttribute : Attribute
    {
        ...
        public IFixAttribute(bool subType = false)
        {
    ...
    public static class Configure
    {
        ...
        //
        public static Dictionary<string, List<TypeConfig>> GetConfigureByTags(List<string> tags)
        {
            var types = from assembly in AppDomain.CurrentDomain.GetAssemblies()
                        ...
                        select type;
            var tagsMap = tags.ToDictionary(t => t, t => new List<TypeConfig>());
                        ...
                        foreach (var ca in prop.GetCustomAttributes(false))
                        {
                            ...
                            if (tagsMap.TryGetValue(ca.GetType().ToString(), out infos))
                            {
                                foreach (var applyTo in prop.GetValue(null, null) as IEnumerable)
                                {
                                    ...
                                    typeConfig.type = applyTo;
                                    typeConfig.flag = flag;
                                    typeConfig.checkSub = false;
                                    if (ca is IFixAttribute) typeConfig.checkSub = ((IFixAttribute)ca).subType;
                                    infos.Add(typeConfig);
                                }
                            }
                        }
                    }
