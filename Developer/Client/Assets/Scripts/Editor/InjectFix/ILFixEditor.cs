...
namespace IFix.Editor
{
    ...
    {
        const string FIX_TOOL_PATH = "/../Tools/IFixToolKit";
        const string CORE_DLL_PATH = "./Assets/Plugins/FramePlus/InjectFix/IFix.Core.dll";
        //备份目录
        ...
        public static void CallIFix(List<string> args)
        {
            ...
            var inject_tool_path = Application.dataPath + FIX_TOOL_PATH + "/IFix.exe";
            //"--runtime = v4.0.30319"
            ...
            hotfix_injection.StartInfo.EnvironmentVariables.Add("workRoot", Application.dataPath + "/../");
        ...
        [MenuItem("Tools/InjectFix/Inject", false, 1)]
        ...
        {
            string pattern = @"MainScripts-(\d{14})\.dll$";
        ...
        public static void InjectAssembly(string assembly)
        {
                ...
                FileAccess.Write)))
            {
                ...
                foreach (var kv in configure)
                {
                    ...
                    List<Configure.TypeConfig> typeList = new List<Configure.TypeConfig>();
                    foreach (var db in kv.Value)
                    {
                        if(!(db.type is Type)) continue;
                        System.Type type = db.type as System.Type;
                        if(type.Assembly.GetName().Name != assembly) continue;
                        typeList.Add(db);
                        if(db.checkSub)
                        {
                            ...
                            {
                                if(types[j].IsSubclassOf(type) && !types[j].IsInterface)
                                {
                                    Configure.TypeConfig tpCfg = new Configure.TypeConfig();
                                    tpCfg.type = types[j];
                                    tpCfg.flag = db.flag;
                                    typeList.Add(tpCfg);
                                }
                    ...
                    {
                        writer.Write(GetCecilTypeName((Type)cfgItem.type));
                        writer.Write(cfgItem.flag);
                        if (filters.Count > 0 && kv.Key == "IFix.IFixAttribute")
                        {
                            foreach(var method in ((Type)cfgItem.type).GetMethods(BindingFlags.Instance 
        ...
        public static void GenPlatformPatch(Platform platform, string patchOutputDir,
            string corePath = CORE_DLL_PATH, int version =0)
        {
            ...
            foreach (var assembly in injectAssemblys)
            {
                GenPatch(assembly, string.Format("{0}/{1}.dll", outputDir, assembly),
                    corePath, string.Format("{0}{1}.patch", patchOutputDir, assembly), version);
            }
#else
        ...
        static void CheckPatchVersion(ref List<Type> vPatchs, System.Type attrType, int version)
        {
            if (attrType == null) return;
            FieldInfo versionField = attrType.GetField("version", BindingFlags.Public | BindingFlags.Instance);
            if (versionField == null) return;
            for (int i = 0; i < vPatchs.Count;)
            {
                Attribute attrObj = vPatchs[i].GetCustomAttribute(attrType);
                if (attrObj == null) continue;
                int defVersion = (int)versionField.GetValue(attrObj);
        ...
        static void CheckPatchVersion<T>(ref List<T> vPatchs, System.Type attrType, int version) where T : MemberInfo
        {
            if (attrType == null) return;
            FieldInfo versionField = attrType.GetField("version", BindingFlags.Public| BindingFlags.Instance);
            if (versionField == null) return;
        ...
        public static void GenPatch(string assembly, string assemblyCSharpPath
            = "Library/ScriptAssemblies/MainScripts.dll", 
            string corePath = CORE_DLL_PATH, string patchPath = "MainScripts.patch", int version = 0)
        {
            ...
            CheckPatchVersion<MethodInfo>(ref patchMethods, typeof(PatchAttribute), version);
            ...
            CheckPatchVersion<MethodInfo>(ref newMethods, typeof(InterpretAttribute), version);
            CheckPatchVersion<FieldInfo>(ref newFields, typeof(InterpretAttribute), version);
            CheckPatchVersion<PropertyInfo>(ref newProperties, typeof(InterpretAttribute), version);
            CheckPatchVersion(ref newClasses, typeof(InterpretAttribute), version);
            ...
            List<string> args = new List<string>() { "-patch", corePath, assemblyCSharpPath, "null",
                processCfgPath, patchPath, version.ToString() };
        ...
        public static void Patch(int version)
        {
            if (!Directory.Exists(Application.dataPath + "/../EditorData/HotScripts"))
            {
                Directory.CreateDirectory(Application.dataPath + "/../EditorData/HotScripts");
            }
            ...
            try
            {
                foreach (var assembly in injectAssemblys)
                {
                    ...
                    GenPatch(assembly, assembly_path, CORE_DLL_PATH,
                        string.Format("EditorData/HotScripts/{0}.patch", assembly), version);
                }
            }
            catch (Exception e)
            {
...
#if UNITY_2018_3_OR_NEWER
        public static void CompileToAndroid(int version)
        {
            if(!Directory.Exists(Application.dataPath + "/../Binarys/HotScripts"))
            {
                Directory.CreateDirectory(Application.dataPath + "/../Binarys/HotScripts");
            }
            EditorUtility.DisplayProgressBar("Generate Patch for Android", "patching...", 0);
            try
            {
                GenPlatformPatch(Platform.android, "Binarys/HotScripts/", CORE_DLL_PATH, version);
            }
            catch(Exception e)
            {
                UnityEngine.Debug.LogError(e);
        ...
        public static void CompileToIOS(int version)
        {
            if (!Directory.Exists(Application.dataPath + "/../Binarys/HotScripts"))
            {
                Directory.CreateDirectory(Application.dataPath + "/../Binarys/HotScripts");
            ...
            try
            {
                GenPlatformPatch(Platform.ios, "Binarys/HotScripts/", CORE_DLL_PATH, version);
            }
            catch(Exception e)
            {
                UnityEngine.Debug.LogError(e);
