...
namespace FMODUnity
{
    ...
    public class EditorUtils : MonoBehaviour
    {
        ...
        public static void PreviewStop(EditorEventRef eventRef)
        {
            if(previewEventInstance.isValid() && previewEventDesc.isValid())
            {
                string eventPath;
                if(previewEventDesc.getPath(out eventPath) == FMOD.RESULT.OK && eventRef.Path.CompareTo(eventPath) ==0)
                    PreviewStop();
            }
        }
