...
namespace Framework.Plugin.AT
{
	...
	{
		[GraphNodeDrawGUIMethod(1169249629)]
		public static bool TopGame_Net_NetWork_Connect_ESessionTypeStringInt32Action_1Int64AServerSession(GraphNode pNode)
		{
			bool bChanged = false;
			...
			if(bChanged)
			{
				...
				{
					Port pvar=null;
					pvar = AgentTreeEditorUtils.BuildOriVariableCommonNew<Framework.Plugin.AT.VariableDelegate>(pVarsFactor, vInTemp, 4);
					pNode.BindNode.AddInPort(pvar);
				}
				{
					Port pvar=null;
					pvar = AgentTreeEditorUtils.BuildOriVariableCommonNew<Framework.Plugin.AT.VariableLong>(pVarsFactor, vInTemp, 5);
					pvar.GetVariable<Framework.Plugin.AT.VariableLong>().mValue = 0;
					pNode.BindNode.AddInPort(pvar);
				}
				{
					Port pvar=null;
		...
		[GraphNodeDrawGUIMethod(-77830448)]
		public static bool TopGame_Net_NetWork_Connect_1_ESessionTypeStringAction_1Int64AServerSession(GraphNode pNode)
		{
			bool bChanged = false;
			...
			if(bChanged)
			{
				...
				{
					Port pvar=null;
					pvar = AgentTreeEditorUtils.BuildOriVariableCommonNew<Framework.Plugin.AT.VariableDelegate>(pVarsFactor, vInTemp, 3);
					pNode.BindNode.AddInPort(pvar);
				}
				{
					Port pvar=null;
					pvar = AgentTreeEditorUtils.BuildOriVariableCommonNew<Framework.Plugin.AT.VariableLong>(pVarsFactor, vInTemp, 4);
					pvar.GetVariable<Framework.Plugin.AT.VariableLong>().mValue = 0;
					pNode.BindNode.AddInPort(pvar);
				}
				{
		...
		[GraphNodeDrawGUIMethod(159717930)]
		public static bool TopGame_Net_NetWork_SendMessage_PacketBaseBoolean(GraphNode pNode)
		{
			bool bChanged = false;
		...
		[GraphNodeDrawGUIMethod(-845519087)]
		public static bool TopGame_Net_NetWork_SendHeart_SingleVoid(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 2)) bChanged = true;
			if(bChanged)
