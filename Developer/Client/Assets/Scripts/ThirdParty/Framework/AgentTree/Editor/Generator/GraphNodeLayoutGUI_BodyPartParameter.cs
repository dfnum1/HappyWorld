...
namespace Framework.Plugin.AT
{
	...
	{
		[GraphNodeDrawGUIMethod(-69107422)]
		public static bool Framework_Core_BodyPartParameter_GetAmiPoint_AInstanceAble(GraphNode pNode)
		{
			bool bChanged = false;
		...
		[GraphNodeDrawGUIMethod(-1293021367)]
		public static bool Framework_Core_BodyPartParameter_ApplayEffect_StringSingleAnimationCurveAnimationCurveAnimationCurveAnimationCurveVoid(GraphNode pNode)
		{
			bool bChanged = false;
			...
			if(bChanged)
			{
				...
				{
					Port pvar=null;
					pvar = AgentTreeEditorUtils.BuildOriVariableCommonNew<Framework.Plugin.AT.VariableCurve>(pVarsFactor, vInTemp, 4);
					pNode.BindNode.AddInPort(pvar);
				}
				{
					Port pvar=null;
					pvar = AgentTreeEditorUtils.BuildOriVariableCommonNew<Framework.Plugin.AT.VariableCurve>(pVarsFactor, vInTemp, 5);
					pNode.BindNode.AddInPort(pvar);
				}
				{
					Port pvar=null;
					pvar = AgentTreeEditorUtils.BuildOriVariableCommonNew<Framework.Plugin.AT.VariableCurve>(pVarsFactor, vInTemp, 6);
					pNode.BindNode.AddInPort(pvar);
				}
				pNode.BindNode.Save();
			}
		...
		[GraphNodeDrawGUIMethod(398127439)]
		public static bool Framework_Core_BodyPartParameter_ApplayRimEffect_StringSingleAnimationCurveVoid(GraphNode pNode)
		{
			bool bChanged = false;
		...
		[GraphNodeDrawGUIMethod(255764216)]
		public static bool Framework_Core_BodyPartParameter_SetPropertyColor_StringColorVoid(GraphNode pNode)
		{
			bool bChanged = false;
		...
		[GraphNodeDrawGUIMethod(-391628369)]
		public static bool Framework_Core_BodyPartParameter_SetPropertyFloat_StringSingleVoid(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 3)) bChanged = true;
			if(bChanged)
