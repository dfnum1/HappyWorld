...
namespace Framework.Plugin.AT
{
	...
	{
		[GraphNodeDrawGUIMethod(-965493104)]
		public static bool TopGame_UI_UIBase_GetUIType_UInt16(GraphNode pNode)
		{
			bool bChanged = false;
		...
		[GraphNodeDrawGUIMethod(-145863365)]
		public static bool TopGame_UI_UIBase_GetOrder_Int32(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 1)) bChanged = true;
			if(!bChanged && (pNode.BindNode.outArgvs == null || pNode.BindNode.outArgvs.Length != 1)) bChanged = true;
		...
		[GraphNodeDrawGUIMethod(1158257097)]
		public static bool TopGame_UI_UIBase_SetOrder_Int32Void(GraphNode pNode)
		{
			bool bChanged = false;
		...
		[GraphNodeDrawGUIMethod(396527448)]
		public static bool TopGame_UI_UIBase_SetZDeepth_Int32Void(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 2)) bChanged = true;
			if(bChanged)
		...
		[GraphNodeDrawGUIMethod(116475848)]
		public static bool TopGame_UI_UIBase_SetPermanent_BooleanVoid(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 2)) bChanged = true;
			if(bChanged)
		...
		[GraphNodeDrawGUIMethod(1102805964)]
		public static bool TopGame_UI_UIBase_GetPermanent_Boolean(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 1)) bChanged = true;
			if(!bChanged && (pNode.BindNode.outArgvs == null || pNode.BindNode.outArgvs.Length != 1)) bChanged = true;
		...
		[GraphNodeDrawGUIMethod(710334278)]
		public static bool TopGame_UI_UIBase_FindLogic_TypeUILogic(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 2)) bChanged = true;
			if(!bChanged && (pNode.BindNode.outArgvs == null || pNode.BindNode.outArgvs.Length != 1)) bChanged = true;
		...
		[GraphNodeDrawGUIMethod(70879939)]
		public static bool TopGame_UI_UIBase_RemoveLogic_UILogicInt32Void(GraphNode pNode)
		{
			bool bChanged = false;
		...
		[GraphNodeDrawGUIMethod(74244052)]
		public static bool TopGame_UI_UIBase_Show_Void(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 1)) bChanged = true;
			if(bChanged)
		...
		[GraphNodeDrawGUIMethod(1021039180)]
		public static bool TopGame_UI_UIBase_Close_Boolean(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 1)) bChanged = true;
			if(!bChanged && (pNode.BindNode.outArgvs == null || pNode.BindNode.outArgvs.Length != 1)) bChanged = true;
		...
		[GraphNodeDrawGUIMethod(-1949292123)]
		public static bool TopGame_UI_UIBase_MoveOutside_Void(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 1)) bChanged = true;
			if(bChanged)
		...
		[GraphNodeDrawGUIMethod(-787964043)]
		public static bool TopGame_UI_UIBase_MoveInside_Void(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 1)) bChanged = true;
			if(bChanged)
		...
		[GraphNodeDrawGUIMethod(-1437928308)]
		public static bool TopGame_UI_UIBase_Hide_Void(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 1)) bChanged = true;
			if(bChanged)
		...
		[GraphNodeDrawGUIMethod(-1613203700)]
		public static bool TopGame_UI_UIBase_IsVisible_Boolean(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 1)) bChanged = true;
			if(!bChanged && (pNode.BindNode.outArgvs == null || pNode.BindNode.outArgvs.Length != 1)) bChanged = true;
		...
		[GraphNodeDrawGUIMethod(920957374)]
		public static bool TopGame_UI_UIBase_CanBreakGuide_Boolean(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 1)) bChanged = true;
			if(!bChanged && (pNode.BindNode.outArgvs == null || pNode.BindNode.outArgvs.Length != 1)) bChanged = true;
		...
		[GraphNodeDrawGUIMethod(1557574079)]
		public static bool TopGame_UI_UIBase_FindOject_StringGameObject(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 2)) bChanged = true;
			if(!bChanged && (pNode.BindNode.outArgvs == null || pNode.BindNode.outArgvs.Length != 1)) bChanged = true;
		...
		[GraphNodeDrawGUIMethod(2093585483)]
		public static bool TopGame_UI_UIBase_Clear_Void(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 1)) bChanged = true;
			if(bChanged)
		...
		[GraphNodeDrawGUIMethod(-689720064)]
		public static bool TopGame_UI_UIBase_LoadObjectAsset_ObjectStringBooleanBooleanSpriteAssetOperiaon(GraphNode pNode)
		{
			bool bChanged = false;
			...
			if(bChanged)
			{
				...
				{
					Port pvar=null;
					pvar = AgentTreeEditorUtils.BuildOriVariableCommonNew<Framework.Plugin.AT.VariableObject>(pVarsFactor, vInTemp, 5);
					pNode.BindNode.AddInPort(pvar);
				}
				{
					Port pvar=null;
		...
		[GraphNodeDrawGUIMethod(-1292562843)]
		public static bool TopGame_UI_UIBase_AddUserParam_StringVariablePoolAbleVoid(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 3)) bChanged = true;
			if(bChanged)
		...
		[GraphNodeDrawGUIMethod(255485600)]
		public static bool TopGame_UI_UIBase_AddUserIntParam_StringInt32Void(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 3)) bChanged = true;
			if(bChanged)
		...
		[GraphNodeDrawGUIMethod(-1605617938)]
		public static bool TopGame_UI_UIBase_AddUserStrParam_StringStringVoid(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 3)) bChanged = true;
			if(bChanged)
		...
		[GraphNodeDrawGUIMethod(-1704230650)]
		public static bool TopGame_UI_UIBase_GetUserParam_StringVariablePoolAble(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 2)) bChanged = true;
			if(!bChanged && (pNode.BindNode.outArgvs == null || pNode.BindNode.outArgvs.Length != 1)) bChanged = true;
		...
		[GraphNodeDrawGUIMethod(-1403380325)]
		public static bool TopGame_UI_UIBase_GetUserIntParam_StringInt32(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 2)) bChanged = true;
			if(!bChanged && (pNode.BindNode.outArgvs == null || pNode.BindNode.outArgvs.Length != 1)) bChanged = true;
		...
		[GraphNodeDrawGUIMethod(472602996)]
		public static bool TopGame_UI_UIBase_GetUserStrParam_StringString(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 2)) bChanged = true;
			if(!bChanged && (pNode.BindNode.outArgvs == null || pNode.BindNode.outArgvs.Length != 1)) bChanged = true;
		...
		[GraphNodeDrawGUIMethod(805696711)]
		public static bool UIBase_Draw_get_view(GraphNode pNode)
		{
			bool bChanged = false;
			if(!bChanged && (pNode.BindNode.inArgvs == null || pNode.BindNode.inArgvs.Length != 1)) bChanged = true;
			if(!bChanged && (pNode.BindNode.outArgvs == null || pNode.BindNode.outArgvs.Length != 1)) bChanged = true;
